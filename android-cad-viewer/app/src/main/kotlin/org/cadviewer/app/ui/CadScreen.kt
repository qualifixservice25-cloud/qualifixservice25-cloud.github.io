...
import org.cadviewer.app.CadUiState
import org.cadviewer.app.CadViewModel
import org.cadviewer.app.R
import org.cadviewer.app.render.CadScene
import org.cadviewer.app.render.CadView
...
private fun DrawingCanvas(
    ...
    AndroidView(
        ...
        update = { view ->
            ...
            view.snapEnabled = state.tool != CadTool.LINE
        },
    )
}
...
private fun statusMessage(state: CadUiState): String {
    val measure = state.quickMeasure?.text(state.formatter)
    return when {
        state.tool.needsPicking && state.remainingPicks > 0 -> {
            val hint = stringResource(
                state.tool.hintRes(state.pendingPoints.size, state.lastPickedSegment != null),
            )
            // La misura appena letta apre la riga: e' il numero per cui l'utente ha toccato
            // lo schermo, e senza questo sparirebbe dietro al suggerimento successivo.
            listOfNotNull(measure, hint, state.snapLabel).joinToString(" · ")
        }
...
private fun CadTool.labelRes(): Int = when (this) {
    ...
    CadTool.LINE -> R.string.tool_line
...
/**
 * Istruzione per il tocco successivo. [collected] sono i punti gia' presi; [hasPickedLine] vale
 * per il solo strumento linea, che non accumula punti ma ricorda l'ultima linea toccata.
 */
private fun CadTool.hintRes(collected: Int, hasPickedLine: Boolean): Int = when (this) {
    ...
    CadTool.LINE -> if (hasPickedLine) R.string.hint_other_line else R.string.hint_line
...
/**
 * Tipi MIME accettati all'apertura: molti file manager marcano i DXF come flusso generico o
 * come testo, quindi i tipi giusti da soli non bastano a far comparire il file nel selettore.
 * Un DWG scelto per sbaglio viene riconosciuto da DrawingRepository.looksLikeDwg e rifiutato
 * con un messaggio, non con un errore di parsing.
 */
